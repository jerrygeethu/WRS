...
{   
      //`scheduleid``supervisorid``description``schfromdate``schtodate``schstatus``schcomment`
	   ...
	   $empreqs= isset($_POST['empreq'])?$_POST['empreq']:null; 
	 ...
	 if(count($empreqs)>0)
	 {
	   ...
	   {
	   $newArrSchEmp=$schempls;
	   }
		$schempls1=$newArrSchEmp;
		 
		
      $schactivity->saveSchData($schid, $suprid,$desc, $schfrom, $schto,$schstatus,$schcomment,$schdepart,$schempls1);
}
...
{		
	window.open(url,'welcome','height=500,width=500,scrollbars=yes,fullscreen=no,location=yes,status=yes');
}
...
function checkall()
{	
	//get Other department employee selected
	var otherdep=document.getElementById('empreq');
	//alert(otherdep);
	if(otherdep!=null)
	{
	...
	for (var i=0; i<obj.options.length; i++)
	{
        ...
        if(document.getElementById('schfromdate').value!='' && document.getElementById('editid').value=='0'  ) {
           ...
           // alert(today);			
			...
			//alert(compareDates(today,'dd/mm/yyyy',fromdate,'dd/mm/yyyy'));
			//alert(today.toString()+" schfrom="+fromdate.toString());
...
//alert("here="+document.getElementById('departmentid').value);
document.location.href="assignschwork.php?dep="+document.getElementById('departmentid').value;
...
<a href="assignschwork.php?schid=0" ><img width="11" border="0" height="14" src="<?php echo $_SESSION['FOLDER'];?>add_new.jpg"/></a>&nbsp;<a href="assignschwork.php?schid=0" >Add new Schedule</a></td>
		...
		<td align="left" >
		<select name="schemployee[]" id="schemployee" size="10" multiple="multiple" title="Scheduled employee list" style='width:150px;height:80px;'><?php echo $schactivity ->get_schemployee($schdepartment,$schactrow['scheduleid']); ?>
		</select>
		</td>
		</tr>
		...
		<!-- select box-->
		 <tr>
			       <td width="200px" colspan="1" align="right" class="Form_txt" >
			        ...
			        // if($_SESSION['TYPE']=='admin'){
			          ...
			          <tr>
						...
						if(isset($schid))
						{												
							$empreq11=$schactivity->get_empreq($schid);
							print $empreq11;										
						}
						?>
			     		</tr>
			     		<tr><td align="right" class="Form_txt">
			     <label for="schfromdate" >Valid from : </label></td>
			     ...
			     <tr>
						...
						<?php
						//show [Request employee] button only if a schedule is created
						if(isset($schid))
						{
							?>
							<input type="button" id="btnreq" title="Request employee" value="Request employee" 
							onclick="javascript:newWindow('emp_request.php?id=<?php echo $schid;?>&dep=<?php echo $departid;?>');"/>
							<?php
						}
						?>
